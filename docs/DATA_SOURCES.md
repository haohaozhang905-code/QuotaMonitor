...
Claude、Codex、WorkBuddy 与 Qoder 的本地文件扫描共用 `LocalTokenScanSupport` 中的缓存读写、文件指纹、JSONL 追加边界、元数据读取和目录枚举基础；来源字段解析、Codex fork 去重、WorkBuddy generation 记录与 Qoder 请求 ID 仍由各自客户端维护。共享基础兼容既有各客户端缓存版本，不代表缓存是完整档案。
...
Codex 的套餐名、额度重置时间、Reset Credit 到期时间是不同字段。当前不提供 ChatGPT Plus 会员到期日；重置机会不会自动使用。服务返回明确到期时间时，每个可用机会会在到期前 24 小时提醒一次；没有到期时间时不推断。具体窗口、数量和时间以服务返回为准。
